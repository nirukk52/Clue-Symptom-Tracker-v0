...
/**
 * QuickEntryPanel - Structured form for rapid symptom logging
 *
 * Why this exists: Spoonies need a low-effort alternative to chatting.
 * This panel lets users log multiple data points (severity, mood, sleep,
 * meds, flare status) in one batch -- inspired by Bearable's quick-log grid.
...
export function QuickEntryPanel() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (severity !== null) {
        entries.push({
          role: 'user',
          content: `Log my overall symptom severity as ${severity}/10 today.`,
        });
      }
      if (mood !== null) {
        entries.push({
          role: 'user',
          content: `Log my mood as ${mood}/10.`,
        });
      }
      if (sleepQuality !== null) {
        entries.push({
          role: 'user',
          content: `Log my sleep quality as ${sleepQuality}/10.`,
        });
      }
      if (medsTaken !== null) {
        entries.push({
          role: 'user',
          content: medsTaken ? 'I took my medications today.' : 'I skipped my medications today.',
        });
      }
      if (isFlare) {
        entries.push({
          role: 'user',
          content: 'Today is a flare day.',
        });
      }
      if (notes.trim()) {
        entries.push({
          role: 'user',
          content: `Quick note: ${notes.trim()}`,
        });
      }

      if (entries.length === 0) return;

      // Send each entry to the chat API to trigger tool calls
  ...
  }, [severity, mood, sleepQuality, medsTaken, isFlare, notes]);
    ...
    <div className="hidden lg:flex flex-col flex-1 bg-white border-l border-primary/6 overflow-hidden">
      ...
      <div className="px-6 py-5 border-b border-primary/6">
        ...
        <p className="text-[12px] text-text-muted leading-relaxed">
          Log your day in under 30 seconds. Tap what applies.
        </p>
      </div>
      ...
      {/* Submit */}
      <div className="px-6 py-4 border-t border-primary/6">
        {saved && (
